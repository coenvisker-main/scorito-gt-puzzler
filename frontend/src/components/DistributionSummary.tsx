...
export function DistributionSummary({ hideComparison = false, hideFormula = false }: DistributionSummaryProps) {
    ...
    return (
        <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-4 flex flex-col gap-4 shadow-xl">
                ...
                <>
                    ...
                    {isGuideOpen && (
                        <div className="bg-neutral-950 border border-neutral-800 rounded-xl p-4 space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                {[
                                    { id: 'α', title: 'Etappegewicht (α)', text: 'Bepaalt hoeveel gewicht de etappewinstkansen krijgen. Verhoog dit als je de teamverdeling puur op etappewinsten wil baseren.' },
                                    { id: 'β', title: 'Kopman-bonus (β)', text: 'Bepaalt de waarde van het type dat vaak de beste kopmankeuze is. Hoe hoger β, hoe meer slots er naar types gaan die vaak als kopman dienen.' },
                                    { id: 'γ', title: 'Trui-multiplier (γ)', text: 'Factor voor dagelijkse klassementsbonussen (D). Standaard gelijk aan het aantal etappes (21).' },
                                    { id: 'δ', title: 'Eindklassement-gewicht (δ)', text: 'Bepaalt hoe zwaar de grote puntenbonussen na de laatste etappe meetellen ten opzichte van dagelijkse punten.' }
                                ].map(item => (
                    ...
                    <div className="space-y-4 pb-4 border-b border-neutral-800/50">
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
                            {[
                                { label: 'Etappe-winst kans', sub: 'α (Gewicht)', key: 'alpha_mult', min: 0, max: 2, step: 0.1, info: 'Hoe zwaar tellen dagelijkse winstkansen mee? (Base: 1.0)' },
                                { label: 'Kopman Waarde', sub: 'β (Gewicht)', key: 'beta_mult', min: 0, max: 2, step: 0.1, info: 'Hoe zwaar weegt het mee dat een type vaak de beste kopmankeuze is? (Base: 15)' },
                                { label: 'Trui Punten', sub: 'γ (Gewicht)', key: 'gamma_mult', min: 0, max: 2, step: 0.1, info: 'Hoe zwaar tellen dagelijkse klassementspunten mee over de hele ronde? (Base: 21)' },
                                { label: 'Eindklassement', sub: 'δ (Gewicht)', key: 'delta_mult', min: 0, max: 2, step: 0.1, info: 'Hoe zwaar tellen de grote puntenbonussen na de laatste etappe mee? (Base: 1.0)' }
                            ].map(param => (
            ...
            {hideFormula && (
                <div className="flex items-center gap-2 text-amber-500 pb-1 border-b border-neutral-800">
                    <BarChart3 className="w-4 h-4" />
                    <span className="text-xs font-black uppercase tracking-[0.1em]">Type Verdeling</span>
                    <span className="text-[10px] text-neutral-500 font-bold uppercase italic ml-auto">Hamilton-allocatie</span>
                </div>
            )}
...
Total TCS: ${score.toFixed(1)}
                    ...
                    return (
                        <div
                            key={type}
                            title={breakdown}
                            className={`flex flex-col min-w-[88px] p-3 rounded-xl border transition-all cursor-help ${
                                hideComparison
                        ...
                        >
                            <span className={`text-[9px] font-black uppercase tracking-widest mb-2 ${actual > 0 || optimal > 0 ? 'text-neutral-400' : 'text-neutral-600'}`}>{type}</span>
                            {hideComparison ? (
                                <div className="flex flex-col gap-0.5">
                                    <span className="text-3xl font-black italic text-white leading-none">{optimal}</span>
                                    <span className="text-[8px] font-bold text-neutral-500 uppercase tracking-widest">Advies</span>
                                </div>
                            ) : (
                                <div className="flex items-end justify-between gap-1">
                                    <span className={`text-3xl font-black italic leading-none ${actual > 0 ? 'text-white' : 'text-neutral-600'}`}>{actual}</span>
                                    <div className="flex flex-col items-end pb-0.5">
                                        <span className="text-xs font-bold text-neutral-500">/{optimal}</span>
                                        {(actual > 0 || optimal > 0) && (
                                            <span className={`text-[9px] font-black px-1.5 py-0.5 rounded-md mt-0.5 ${
                                                isOk ? 'text-emerald-400 bg-emerald-500/15' : diff > 0 ? 'text-amber-400 bg-amber-500/15' : 'text-red-400 bg-red-500/15'
                                            }`}>
